...
int main(int argc, char *argv[]) {
  ...
  auto f_input = TFile::Open((base_dir + "/data/examples/ttbar_testing_170_gev_klfinput.root").c_str(), "READ");
  if (f_input == nullptr) {
  ...
  if (!fitter.SetLikelihood(&likelihood)) {
    std::cerr << "ERROR: Failed to set likelihood. Abortievent.lepton_ptng." << std::endl;
    return 1;
  }
  ...
  auto f_output = TFile::Open("ttbar_testing_170_gev_klfoutput.root", "RECREATE");
 ...
 //   if (((event.jet_pt->size())>6))
   ...
   // }
    ...
    float sumet = event.sumet;
    // sumet=0;
    // for (int addsumet=0;addsumet<event.njet;addsumet++)
    // {
    //   sumet=sumet+event.jet_pt[addsumet];
    //   }

    // Add MET information
    const double met_ex = event.met_met*cos(event.met_phi);
    const double met_ey = event.met_met*sin(event.met_phi);
    if (!fitter.SetET_miss_XY_SumET(met_ex, met_ey, sumet)) {
