...
unit = 'pb'
nbins=5
bkg11 = np.load('span_bkg_bin_pretraining_nbins'+str(nbins)+'.npy')
sig11 = np.load('span_sig_bin_pretraining_nbins'+str(nbins)+'.npy')
...
fig.show()
plt.title("SPANet pretraining, expected limits:"+str(np.round(exp_limits[2],3))+" bkg uncertainty: 20% (300fb-1)")
fig.savefig("SPANetpretraining.png")
...
print("pretraining:"+str(np.round(exp_limits[2],3))+" xsec:"+str(np.round((np.round(exp_limits[2],3)*SMxec),3))+"(pb) "+ f'qtilde = {teststat}')
