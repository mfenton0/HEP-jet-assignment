...
unit = 'pb'
nbins=5
bkg11 = np.load('klf_bkg_bin_nbins'+str(nbins)+'.npy')
sig11 = np.load('klf_sig_bin_nbins'+str(nbins)+'.npy')
...
if nbins==5:
    ...
    mai = mai*140/139
    nai = nai*140/139
    print("Run2 style, LUM = 140fb^-1")
else:
    bkgrate = 0.3
    mai = mai*300/139
    nai = nai*300/139
    print("Run3 style, LUM = 300fb^-1 ")
print("BG uncertainty: "+str(bkgrate))
print("kttbb: 1.5")
uncer = []
sig1 = []
bkg1 = []
...
fig.show()
plt.title("KLFitter BDT, expected limits:"+str(np.round(exp_limits[2],3))+" bkg uncertainty: 20% (300fb-1)")
fig.savefig("senseklf.png")
...
print("KLFBDT:"+str(np.round(exp_limits[2],3))+" xsec:"+str(np.round((np.round(exp_limits[2],3)*SMxec),3))+"(pb) "+ f'qtilde = {teststat}')