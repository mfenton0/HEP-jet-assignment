...
int main(int argc, char *argv[]) {
  ...
  auto f_input = TFile::Open((base_dir + "/data/examples/BDTspanet_ttbbinput.root").c_str(), "READ");
  if (f_input == nullptr) {
  ...
  KLFitter::LikelihoodTTHLeptonJets likelihood{};

  // Set the likelihood properties.
  likelihood.PhysicsConstants()->SetMassTop(172.5); 
  likelihood.PhysicsConstants()->SetMassHiggs(125);  // mass in GeV
  ...
  likelihood.SetFlagHiggsMassFixed(true);
  ...
  std::vector<float> klf_hquark1_pt;
  std::vector<float> klf_hquark1_eta;
  std::vector<float> klf_hquark1_phi;
  std::vector<float> klf_hquark1_e;
  std::vector<unsigned int> klf_hquark1_jet_index;
  std::vector<float> klf_hquark2_pt;
  std::vector<float> klf_hquark2_eta;
  std::vector<float> klf_hquark2_phi;
  std::vector<float> klf_hquark2_e;
  std::vector<unsigned int> klf_hquark2_jet_index;
  ...
  out_tree.Branch("klf_hquark1_pt", &klf_hquark1_pt);
  out_tree.Branch("klf_hquark1_eta", &klf_hquark1_eta);
  out_tree.Branch("klf_hquark1_phi", &klf_hquark1_phi);
  out_tree.Branch("klf_hquark1_e", &klf_hquark1_e);
  out_tree.Branch("klf_hquark1_jet_index", &klf_hquark1_jet_index);
  out_tree.Branch("klf_hquark2_pt", &klf_hquark2_pt);
  out_tree.Branch("klf_hquark2_eta", &klf_hquark2_eta);
  out_tree.Branch("klf_hquark2_phi", &klf_hquark2_phi);
  out_tree.Branch("klf_hquark2_e", &klf_hquark2_e);
  out_tree.Branch("klf_hquark2_jet_index", &klf_hquark2_jet_index);
  ...
  for (Long64_t ievent = 0; ievent < nEntries; ievent++) {
    ...
    klf_hquark1_pt.clear();
    klf_hquark1_eta.clear();
    klf_hquark1_phi.clear();
    klf_hquark1_e.clear();
    klf_hquark1_jet_index.clear();
    klf_hquark2_pt.clear();
    klf_hquark2_eta.clear();
    klf_hquark2_phi.clear();
    klf_hquark2_e.clear();
    klf_hquark2_jet_index.clear();
    klf_lepton_pt.clear();
    ...
    if (event.Lepton_is_e) {
      likelihood.SetLeptonType(KLFitter::LikelihoodTTHLeptonJets::kElectron);
      particles.AddParticle(lepton, event.Lepton_cl_eta, KLFitter::Particles::kElectron);
    } else if (event.Lepton_is_mu) {
      likelihood.SetLeptonType(KLFitter::LikelihoodTTHLeptonJets::kMuon);
    ...
    for (int ijet = 0; ijet < 6; ijet++) {
       ...
       TLorentzVector jet;
       jet.SetPtEtaPhiM(event.Jet_pt->at(ijet), event.Jet_eta->at(ijet),
          event.Jet_phi->at(ijet), event.Jet_m->at(ijet));
 ...
 //   if (((event.jet_pt->size())>6))
   // {
    // for (unsigned int ijet = 0; ijet < 7; ijet++) {
     // TLorentzVector jet;
   ...
   // }
    ...
    //std::cout<<event.jet_eta.at(ijet)<<std::endl;
    ...
    //float sumet;
    //sumet=0;
    //for (int addsumet=0;addsumet<8;addsumet++)
    //{
      //sumet=sumet+event.Jet_pt->at(addsumet);
     // }
    // Add MET information
    ...
    for (int iperm  = 0; iperm < nperm; iperm++) {
      ...
      double event_probability = fitter.Likelihood()->LogEventProbability();
      ...
      // higgs quark 1.
      float hquark1_pt = modelParticles->Parton(4)->Pt();
      float hquark1_eta = modelParticles->Parton(4)->Eta();
      float hquark1_phi = modelParticles->Parton(4)->Phi();
      float hquark1_e = modelParticles->Parton(4)->E();
      unsigned int hquark1_index = (*permutedParticles)->JetIndex(4);

      // higgs quark 2.
      float hquark2_pt = modelParticles->Parton(5)->Pt();
      float hquark2_eta = modelParticles->Parton(5)->Eta();
      float hquark2_phi = modelParticles->Parton(5)->Phi();
      float hquark2_e = modelParticles->Parton(5)->E();
      unsigned int hquark2_index = (*permutedParticles)->JetIndex(5);
      ...
      klf_hquark1_pt.emplace_back(hquark1_pt);
      klf_hquark1_eta.emplace_back(hquark1_eta);
      klf_hquark1_phi.emplace_back(hquark1_phi);
      klf_hquark1_e.emplace_back(hquark1_e);
      klf_hquark1_jet_index.emplace_back(hquark1_index);
      klf_hquark2_pt.emplace_back(hquark2_pt);
      klf_hquark2_eta.emplace_back(hquark2_eta);
      klf_hquark2_phi.emplace_back(hquark2_phi);
      klf_hquark2_e.emplace_back(hquark2_e);
      klf_hquark2_jet_index.emplace_back(hquark2_index);
