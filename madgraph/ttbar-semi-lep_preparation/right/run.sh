...
for i in {1..1};do
	if [ $i == 1 ]
		...
		python3 $ANALYSIS_SCRIPT_PATH/main.py -p 1 -u parse -m ttbar_lep_right -i $ROOT_FILE_PATH/run_0$i/tag_1_delphes_events.root -o /home/david/mass_generation/event_record_top_lep_right_$i.npz -s 1 >> $LOG_FILE_PATH/log.txt
...
#	./bin/mg5_aMC pptt_lep.txt >> $LOG_FILE_PATH/log.txt
#	echo "Parsing root file generate by pythia8 showering."
#	python3 $ANALYSIS_SCRIPT_PATH/main.py -p 1 -u parse -m ttbar_lep_right -i $ROOT_FILE_PATH/run_$i/tag_1_delphes_events.root -o /home/david/mass_generation/event_record_top_lep_right_$i.npz -s 1 >> $LOG_FILE_PATH/log.txt
#	echo "Removing generate files."
